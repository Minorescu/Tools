...
##
## License: GPL
## It can reinstall Debian, Ubuntu, Kali, Alpine, CentOS, AlmaLinux, RockyLinux, Fedora and Windows OS via network automatically without any other external measures and manual operations.
## Default root password: Teddysun.com
...
function checkEfi() {
	EfiStatus=$(efibootmgr l 2>/dev/null)
	EfiVars=""
...
function checkSys() {
	_exists "apt" && apt update -y
	_exists "apt" && apt install lsb-release -y
	_exists "yum" && yum install redhat-lsb -y
	_exists "apk" && apk update
	...
	_exists "apt" && apt install cpio curl dnsutils efibootmgr fdisk file gzip jq net-tools openssl subnetcalc tuned wget xz-utils -y

	# Redhat like Linux OS prefer to use dnf instead of yum because former has a higher execute efficiency.
	_exists "yum" && yum install epel-release -y
	_exists "yum" && yum install dnf -y
	if [[ $? -eq 0 ]]; then
	...
	[[ "$CurrentOS" == "AlpineLinux" ]] && {
		...
		_exists "apk" && apk update
		_exists "apk" && apk add bash bind-tools coreutils cpio curl efibootmgr file gawk grep gzip ipcalc jq net-tools openssl sed shadow tzdata util-linux wget xz
...
[[ -z "$tmpWORD" || "$linux_relese" == 'alpinelinux' || "$targetRelese" == 'Ubuntu' ]] && tmpWORD='Teddysun.com'
myPASSWORD=$(openssl passwd -1 ''$tmpWORD'')
[[ -z "$myPASSWORD" ]] && myPASSWORD='$1$J2R7PQgJ$gweAgNfNZmTetM0uGGcO11'
...
EOF
	...
	if [[ "$linux_relese" == 'centos' ]] || [[ "$linux_relese" == 'rockylinux' ]] || [[ "$linux_relese" == 'almalinux' ]]; then
		if [[ "$RedHatSeries" -ge "8" ]]; then
			...
			[[ "$IsCN" == "cn" ]] && RepoEpel="repo --name=epel --baseurl=http://mirror.nju.edu.cn/epel/$RedHatSeries/Everything/${VER}/" || RepoEpel="repo --name=epel --baseurl=https://archives.fedoraproject.org/pub/epel/$RedHatSeries/Everything/${VER}/"
		elif [[ "$linux_relese" == 'centos' ]] && [[ "$RedHatSeries" -le "7" ]]; then
			...
			[[ "$IsCN" == "cn" ]] && RepoEpel="repo --name=epel --baseurl=http://mirror.nju.edu.cn/epel/$RedHatSeries/${VER}/" || RepoEpel="repo --name=epel --baseurl=https://archives.fedoraproject.org/pub/archive/epel/$RedHatSeries/${VER}/"
		fi
	elif [[ "$linux_relese" == 'fedora' ]]; then
